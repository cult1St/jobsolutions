...
{
    public function signup($fname, $email, $password, $compname){
       ...
       try{
        $sql = "INSERT INTO employers(employer_fullName, employer_email, employer_password, employer_companyName) VALUES(?, ?, ?, ?)";
        $stmt = $this->dbconn->prepare($sql);
        $stmt->execute(array($fname, $email, $password, $compname));
    ...
    public function post_job($title, $qualification, $employer_id, $dateclosed, $description, $salary, $jobcatid, $type, $state, $lga){
        try{
            ...
            $stmt = $this->dbconn->prepare($sql);
           $result =  $stmt->execute(array($title, $qualification, $employer_id, $dateclosed, $description, $salary, $jobcatid, $type, $state, $lga));
           return $result;
    ...
    public function fetch_vacancies_for_users_by_id($id){
        try{
            $sql = "SELECT * FROM `job_vacancy` JOIN employers ON jobVacancy_employerId = employer_id JOIN state ON states_id =state_id JOIN lga ON lga = lga_id JOIN job_category ON job_vacancy.jobCatId = job_category.jobCatId WHERE jobVacancy_id=? ";
            $stmt = $this->dbconn->prepare($sql);
            $stmt->execute([$id]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function get_current_jobs(){
        $sql = "SELECT * FROM `job_vacancy` JOIN employers ON jobVacancy_employerId = employer_id JOIN state ON states_id =state_id JOIN lga ON lga = lga_id WHERE job_vacancy.dateClosed >= ?";
        $stmt = $this->dbconn->prepare($sql);
